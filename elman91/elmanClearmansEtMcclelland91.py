...
python elmanServanSchreiberEtAl.py 60000 6-80-150 50 3-15 0.01-0.015-0.02 0.1-0.5-0.7-0.9

Parameter1 : List of Number of sequences separated by -
Parameter2 : List of sizes of sequences separated by -
Parameter3 : List of Window of average separated by -
Parameter4 : List of Number of neurons separated by -
Parameter5 : List of Learning rate separated by -
Parameter6 : List of Momentum separated by -
...
def elmanAnalysis(data,index_data, sequences ,window, nbNeuronHiddenLayer,learningRate, momentum):
    ...
    network = Elman(6, nbNeuronHiddenLayer, 6)
    errors = []
    mean_error = []
    index = 0
    counter =0
    ...
    plt.figtext(.05, .05, '\nNumber of sequences : ' + str(NumberOfSequences) +'\nSize of sequences : ' + str(SequenceSize) + '\nNumber of samples : ' + str(
        nbSamples) + '\nAverage of the error every ' + str(window) + ' results'+ '\nLearning rate ' + str(learningRate)+ '\nMomentum ' + str(momentum) )

    plt.savefig(str(index_data)+"_NetRes_NbSeq" + str(NumberOfSequences)+"-SizSeq" + str(SequenceSize) + "-Win" + str(window) + "-lr" + str(learningRate) + "-mom" + str(momentum) + ".png")  # save as png
    #plt.show()
...
if __name__ == '__main__':
    ...
    if len(sys.argv) <=1:
        ...
        listSequenceSize =  sys.argv[2].split("-")
        listwindows = sys.argv[3].split("-")
        listnbNeuronHiddenLayer = sys.argv[4].split("-")
        listlearningRate= sys.argv[5].split("-")
        listmomentum = sys.argv[6].split("-")

    print "List of all the parameters : "
    print "List of Number of sequences : ", listNumberOfSequences
    print "List of sizes of sequences : ", listSequenceSize
    ...
    parameters.append(listSequenceSize)
    ...
    for NumberOfSequences in listNumberOfSequences:
        ...
        for SequenceSize in listSequenceSize:
            ...
            nbSamples = 0
            LenSequences = []
            sequences = generate_sequences(n=int(NumberOfSequences), length=int(SequenceSize))
                ...
                for nbNeuronHiddenLayer in listnbNeuronHiddenLayer:
                    for learningRate in listlearningRate:
                        for momentum in listmomentum:
                            ...
                            data[index_data]["sizeOfSequence"] = SequenceSize
